...
public class QuizTAActivity extends AppCompatActivity {
    ...
    MediaPlayer player;
    TextView mTextTv,btnPass1,tvQ;
    ImageButton mVoiceBtn;
    ImageView iv;
    boolean check = false;
    public static int corCount = 0;
    public static int questionCount = 50;
    int curNum = 0; // 0,1,2 ... questionCount-1
    public static int count = Fragment1.year;
    public static int[] songNums = new int[questionCount]; // 랜덤으로 10곡 저장
    int[][] music = {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_quiz_ta);

        mTextTv = findViewById(R.id.textTA);
        mVoiceBtn = findViewById(R.id.voiceBtnTA);
        btnPass1 = findViewById(R.id.btnPassTA);
        tvQ = findViewById(R.id.tvQT);

        iv = (ImageView)findViewById(R.id.ivTA);
        ...
        btnPass1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(curNum == questionCount -1){
                    player.stop();
                    player = null;
                    Intent intent = new Intent (QuizTAActivity.this, ResultTAActivity.class);
    ...
    public void countDownTimer(){
        Glide.with(QuizTAActivity.this).load(R.drawable.wavesky).into(iv);
        countDownTimer = new CountDownTimer(TOTAL, COUNT_DOWN_INTERVAL) {
            @Override
            public void onTick(long millisUntilFinished) {
                Glide.with(QuizTAActivity.this).load(R.drawable.countdownsky).into(iv);
                mVoiceBtn.setClickable(false);
                btnPass1.setClickable(false);
            }
            @Override
            public void onFinish() {
                mVoiceBtn.setClickable(true);
                btnPass1.setClickable(true);
            }
        };
    }
    ...
    public void delayMusicPlay() {
        ...
        handler.postDelayed(new Runnable(){
            ...
            public void run() {
                Glide.with(QuizTAActivity.this).load(R.drawable.wavesky).into(iv);
                player = MediaPlayer.create(QuizTAActivity.this, music[count][songNums[curNum]]);
                player.start();
                player.setLooping(true);
            }
        },4000);
    ...
    public void countDown(){
        countDown = new CountDownTimer(180000, COUNT_DOWN_INTERVAL) {
            int sec = 180;
            @Override
            public void onTick(long millisUntilFinished) {
                mTextTv.setText(sec+"초");
                sec--;
            }
            @Override
            public void onFinish() {
